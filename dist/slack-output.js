...
class SlackOutput {
    ...
    async handleToolExecution(response, metadata) {
        ...
        let blocks;
        try {
            blocks = this.createToolBlocks(toolName, toolStatus, duration, toolUse.input, toolResult);
        }
        catch (error) {
     ...
     */
    async finalizeSessionWithoutResult() {
        if (!this.initialMessageTs)
            return;
        try {
            // Remove rocket emoji
            ...
            this.logSlackCall('reactions.remove', removePayload);
            await this.rateLimiter.execute(() => this.client.reactions.remove(removePayload));
            ...
            this.logSlackCall('reactions.add', addPayload);
            await this.rateLimiter.execute(() => this.client.reactions.add(addPayload));
        }
        catch (error) {
